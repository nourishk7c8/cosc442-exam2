...
public class AttackOption extends ToHitData {
    ...
    static class Sorter implements Comparator<AttackOption> {
		...
		private int wIsMissile(AttackOption a, AttackOption a1, WeaponType w1) {
    ...
    public AttackOption(CEntity target, Mounted weapon, double value,
        ...
        if (target != null && weapon != null) {
            ...
            setPrimaryValues(toHit);

            setOdds(target, toHit, sec_mod);
            heat = w.getHeat();
            ...
            checkAmmo(weapon, usesAmmo);
        }
    }

	private void checkAmmo(Mounted weapon, final boolean usesAmmo) {
	...
	private void setOdds(CEntity target, ToHitData toHit, int sec_mod) {
	...
	private void setPrimaryValues(ToHitData toHit) {
